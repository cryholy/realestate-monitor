...
5. Notion에 새 하위 페이지를 만들고 비공개 또는 숨겨진 초안 상태로 둔다.
6. 제목은 `2026-06-01 서울 한강권 대표단지 갭 레이더` 형식으로 쓴다.
7. Markdown 본문을 Notion 페이지에 붙여넣는다.
8. CSV 파일은 필요하면 파일 첨부 또는 별도 database import로 올린다.
9. 일요일 22:30~23:30 KST에 요약, 표, 주의사항을 검수한다.
10. 월요일 07:30 KST에 공개 Notion Site 메인 페이지의 `최신 주간 리포트` 링크를 새 페이지로 교체한다.
11. `지난 리포트 아카이브`에 새 페이지 링크를 추가한다.
