...
# ---------------------------------------------------------------------------
# v2: 신규 전세 분리 + 갱신 압력 신호 + 신뢰도 가중치 정렬
# v1 코드는 위 영역. 1~2주 운영 검증 후 별도 PR로 v1 영역 제거 예정.
# ---------------------------------------------------------------------------
...
class RadarRowV2:
    ...
    @classmethod
    def from_db(cls, row: dict) -> "RadarRowV2":
        return cls(
            district_rank=int(row.get("district_rank") or 0),
            representative_score=float(row.get("representative_score") or 0),
            apt_seq=row.get("apt_seq") or "",
...
def render_markdown_report_v2(rows: list[RadarRowV2], *, report_date: str) -> str:
    rows = sorted(
        rows,
    ...
    lines.extend(_render_v2_delta_table(gap_top, metric="gap"))
    ...
    lines.extend(_render_v2_delta_table(ratio_top, metric="ratio"))
    ...
    lines.extend(_render_v2_delta_table(use_top, metric="use_rate"))
    ...
    lines.extend([
        ...
        "- 보정점수는 변동률의 절대값에 신뢰도 가중치(높음 1.0 / 주의 0.5 / 데이터 부족 0.0)를 곱한 값으로, 표본이 적은 변동의 영향을 자연스럽게 줄입니다.",
        "- 실거래 신고 지연으로 최근 거래는 뒤늦게 추가될 수 있습니다.",
        "- 이 리포트는 매수 추천이나 투자 추천이 아닙니다.",
        "",
    ])
...
def _render_v2_delta_table(rows: list[RadarRowV2], *, metric: str) -> list[str]:
    if metric == "gap":
        header = "| 생활권 | 구 | 단지 | 평형 | 현재 갭 | 직전 갭 | 변화 | 보정점수 | 신뢰도 |"
        align = "|---|---|---|---:|---:|---:|---:|---:|---|"
    elif metric == "ratio":
        header = "| 생활권 | 구 | 단지 | 평형 | 현재 전세가율 | 직전 전세가율 | 변화 | 보정점수 | 신뢰도 |"
        align = "|---|---|---|---:|---:|---:|---:|---:|---|"
    elif metric == "use_rate":
        header = "| 생활권 | 구 | 단지 | 평형 | 현재 사용률 | 직전 사용률 | 변화 | 신규-갱신 갭 | 보정점수 | 신뢰도 |"
        align = "|---|---|---|---:|---:|---:|---:|---:|---:|---|"
    else:
        raise ValueError(f"unknown metric: {metric}")
...
def render_csv_v2(rows: list[RadarRowV2]) -> str:
    out = StringIO()
    writer = csv.DictWriter(out, fieldnames=[
        "district_name",
        "apt_name",
        ...
        "representative_score",
        "sale_count_12m",
        "jeonse_new_count_12m",
        "sale_median_90d",
        "jeonse_new_median_90d",
        "gap_90d",
        "jeonse_ratio_90d",
        "jeonse_renewal_median_90d",
        "renewal_right_use_rate_90d",
        "renewal_pct_change_median_90d",
        "new_minus_renewal_gap_90d",
        "gap_delta",
        "ratio_delta",
        "use_rate_delta",
        "weighted_gap_delta",
        "weighted_ratio_delta",
        "weighted_use_rate_delta",
        "reliability_label",
        "reliability_weight",
        "apt_seq",
    ])
    ...
    for row in rows:
        writer.writerow({
            ...
            "representative_score": row.representative_score,
            "sale_count_12m": row.sale_count_12m,
            "jeonse_new_count_12m": row.jeonse_new_count_12m,
            "sale_median_90d": row.sale_median_90d,
            "jeonse_new_median_90d": row.jeonse_new_median_90d,
            "gap_90d": row.gap_90d,
            "jeonse_ratio_90d": row.jeonse_ratio_90d,
            "jeonse_renewal_median_90d": row.jeonse_renewal_median_90d,
            "renewal_right_use_rate_90d": row.renewal_right_use_rate_90d,
            "renewal_pct_change_median_90d": row.renewal_pct_change_median_90d,
            "new_minus_renewal_gap_90d": row.new_minus_renewal_gap_90d,
            "gap_delta": row.gap_delta,
            "ratio_delta": row.ratio_delta,
            "use_rate_delta": row.use_rate_delta,
            "weighted_gap_delta": row.weighted_gap_delta,
            "weighted_ratio_delta": row.weighted_ratio_delta,
            "weighted_use_rate_delta": row.weighted_use_rate_delta,
