-- REM 갭 레이더 — 단일 view 정의 통합
-- 옛 v1(_v1 없음) view + 옛 v2(_v2 접미) view 모두 DROP 후 단일 정의로 재생성.

-- 1) 옛 v1 view DROP (007에서 정의됐던 것)
DROP VIEW IF EXISTS v_gap_radar_weekly_rows CASCADE;
DROP VIEW IF EXISTS v_gap_radar_representative_items CASCADE;
DROP VIEW IF EXISTS v_gap_radar_candidate_scores CASCADE;
DROP VIEW IF EXISTS v_gap_radar_rent_12m CASCADE;
DROP VIEW IF EXISTS v_gap_radar_sale_12m CASCADE;

-- 2) 옛 v2 view DROP (008에서 정의됐던 것)
DROP VIEW IF EXISTS v_gap_radar_weekly_rows_v2 CASCADE;
DROP VIEW IF EXISTS v_gap_radar_representative_items_v2 CASCADE;
DROP VIEW IF EXISTS v_gap_radar_candidate_scores_v2 CASCADE;
DROP VIEW IF EXISTS v_gap_radar_jeonse_new_12m_v2 CASCADE;
DROP VIEW IF EXISTS v_gap_radar_sale_12m_v2 CASCADE;
DROP VIEW IF EXISTS v_jeonse_renewal_v2 CASCADE;
DROP VIEW IF EXISTS v_jeonse_new_v2 CASCADE;
DROP VIEW IF EXISTS v_sale_clean_v2 CASCADE;

-- 3) 단일 정의 재생성 — 정제 base
CREATE OR REPLACE VIEW v_sale_clean
...
CREATE OR REPLACE VIEW v_jeonse_new
WITH (security_invoker = true) AS
SELECT *
FROM rent_records
...
CREATE OR REPLACE VIEW v_jeonse_renewal
WITH (security_invoker = true) AS
SELECT *
FROM rent_records
...
CREATE OR REPLACE VIEW v_gap_radar_jeonse_new_12m
WITH (security_invoker = true) AS
SELECT
  r.apt_seq,
  r.size_label,
...
WITH joined AS (
  ...
  LEFT JOIN v_gap_radar_jeonse_new_12m r
